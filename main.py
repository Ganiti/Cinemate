...
def rcmd(m):
    ...
    if m not in data['movie_title'].unique():
        ...
        return l
#finding out the top 8 trending movies
def fun():
    import pandas as pd
    ...
    for i in range(8):
        curr_max=-1
        curr_ind=-1
        for j in range(len(df['num_voted_users'])):
            if j in max_indexes:
                continue
            if curr_max<df['num_voted_users'][j]:
                curr_max=df['num_voted_users'][j]
                curr_ind=j
        max_indexes.append(curr_ind)
    for i in max_indexes:
        arr_pop.append(df.iloc[i]['movie_title'])
